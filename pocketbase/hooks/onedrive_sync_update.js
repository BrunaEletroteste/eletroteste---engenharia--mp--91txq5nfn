onRecordAfterUpdateSuccess((e) => {
  ...
  let oldFotos = e.record.original() ? e.record.original().get('fotos') : []
  if (!oldFotos) oldFotos = []
  if (!Array.isArray(oldFotos)) oldFotos = [oldFotos]

  const photosToUpload = newFotos.filter(
    (f) => typeof f === 'string' && f.trim() !== '' && !oldFotos.includes(f),
  )
