onRecordAfterUpdateSuccess((e) => {
  ...
  const logAudit = (message) => {
    try {
      ...
      const auditRecord = new Record(auditLogs)
      auditRecord.set('user', userId)
      auditRecord.set('action_type', 'edicao')
      auditRecord.set('details', message)
      $app.save(auditRecord)
    } catch (auditErr) {
      $app
        .logger()
        .error('Failed to create audit log for OneDrive sync failure', 'error', String(auditErr))
    }
  }
  ...
  try {
    ...
    if (tokenRes.statusCode !== 200) {
      ...
      logAudit('Falha na sincronização OneDrive (token): status ' + tokenRes.statusCode)
      return e.next()
    }
    ...
    for (const fileName of filesToUpload) {
      ...
      try {
        ...
        if (fileRes.statusCode !== 200) {
          $app
            .logger()
            .error('Failed to download file from PB for update sync', 'fileName', fileName)
          continue
        }
        ...
        $app
          .logger()
          .error('Error syncing file to OneDrive', 'fileName', fileName, 'error', String(fileErr))
      }
    }
    ...
    $app
      .logger()
      .error(
        'OneDrive sync failed for relatorio update',
        'relatorioId',
        record.id,
        'error',
        String(err),
      )
    logAudit('Falha na sincronização OneDrive: ' + String(err))
  }
